...
def req(**kwargs):
    base = {
        "request_id": "r1",
        "identity": Identity("u", "analyst", 95),
        "agent_id": "a1",
...
def test_unknown_capability_is_blocked():
    assert ShieldEngine().evaluate(req(capability="delete_everything")).decision == Decision.BLOCK


def test_role_escalation_is_blocked():
    assert ShieldEngine().evaluate(req(capability="execute_transaction")).decision == Decision.BLOCK
...
def test_external_egress_defaults_to_deny():
    assert ShieldEngine().evaluate(req(external_network=True, destination="example.invalid")).decision == Decision.BLOCK
...
def test_isolation_revokes_capabilities():
    e = ShieldEngine()
    e.emergency_isolate("synthetic adversarial event")
    assert e.evaluate(req()).decision == Decision.ISOLATE
...
def test_provenance_tampering_is_detected():
    e = ShieldEngine()
    e.evaluate(req())
    e.provenance.events[0].resource = "tampered"
